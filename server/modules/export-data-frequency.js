...
let exportFrequencyData = function (query, modifier, callback) {

  let archive = _initializeZipArchive();
  _compileFrequencyZip( archive, query, modifier );

  return _generateZipArchive( archive );
};
let _compileFrequencyZip = ( archive, query, modifier) => {
  _prepareFreqDataForArchive( archive, SkillsFrequency, 'csv', 'frequency-data.csv', query, modifier );
};
let _prepareFreqDataForArchive = ( archive, collection, type, fileName, query, modifier ) => {
  let data          = collection instanceof Mongo.Collection ? _getFreqDataFromCollection( collection, query, modifier ) : collection,
      formattedData = _formatData[ type ]( data );
  _addFileToZipArchive( archive, fileName, formattedData );
};
let _getFreqDataFromCollection = ( collection, query, modifier ) => {
  var queryMods = {
    // trackedSkills: {
    //   $ne: []
    // }
...
Modules.server.exportFrequencyData = exportFrequencyData;