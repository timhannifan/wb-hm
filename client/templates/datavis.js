...
Template.datavis.onCreated( () => {
	...
	Tracker.autorun(() => {
		...
		var subHandle1 = Meteor.subscribe('JSAggs');
		var subHandle2 = Meteor.subscribe('JobStreetMeta');
		// var subHandle2 = Meteor.subscribe('Skills.types');
		var subHandle3 = Meteor.subscribe('Skills.byType', instance.getSkillFilter() );
	  	const isReady = subHandle1.ready() && subHandle2.ready() && subHandle3.ready();
...
Template.datavis.onRendered( () => {
	...
	Deps.autorun(function(){
		var skillType = instance.getSkillFilter();
 		var data = [];
	  	var skillsCursor = () => {
	  		if (skillType == 'all') {
	  			return Skills.find({}, {sort: {parsed_keyword: 1}});
	  		} else {
	  			return Skills.find({type: skillType}, {sort: {parsed_keyword: 1}});
	  		}
	  	};
	  	var skillsKeywordMap = skillsCursor().map(function(doc){return doc.parsed_keyword});
	  	var aggsCursor = JSAggs.find({}, {});
	  	var aggsMap = aggsCursor.map(function(doc){return doc});

	  	var hccol = skillsCursor().map(function(doc,index){
	  	  		return index+1;
	  	  	}),
 		...
 		var heatMap = svg.append("g").attr("class","g3")
  		...
  		function sortbylabel(rORc,i,sortOrder){
  		   ...
  		   if(rORc=="r"){ // sort by col
  		     sorted=d3.range(skillsCursor().count()).sort(function(a,b){ if(sortOrder){ return frequencies[b]-frequencies[a];}else{ return frequencies[a]-frequencies[b];}});
  		     t.selectAll(".cell")
  		     ...
  		     sorted=d3.range(aggsCursor.count()).sort(function(a,b){if(sortOrder){ return frequencies[b]-frequencies[a];}else{ return frequencies[a]-frequencies[b];}});
  		     t.selectAll(".cell")
...
Template.datavis.events({
	'change #jsSkillType': function (event, instance) {
		...
		var svg = d3.select("#chart");
		var t = svg.selectAll(".cellg").remove();
		// var self = this;
		// var newVal = ;
